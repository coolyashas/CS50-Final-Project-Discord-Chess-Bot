...
from dbs import Solving, Solved, Games
...
class Tactics(commands.Cog):
    ...
    async def details(self, ctx):
        await ctx.response.send_message("""Title: Discord Chess Bot\nCreator: Yashas Donthi\nMade with love in Bangalore, India :heart:""")

    @slash.command(name="solve", description="Make your move to solve the puzzle")
    async def solve(self, ctx, move: str):
        await ctx.response.send_message(content="okey",delete_after=0.0001,ephemeral=True)
        global scores

        results = self.session.query(Solving).filter(Solving.channel_id==ctx.channel.id, Solving.user_id==ctx.user.id).order_by(Solving.index.desc()).first()
        ...
        if move != movelist[0]:
            ...
            self.session.query(Solving).filter(Solving.channel_id==ctx.channel.id, Solving.user_id==ctx.user.id).update({Solving.board: board, Solving.movelist: movelist})            
            await self.render(message_id, ctx.channel.id)

            if len(movelist)==0:
                await ctx.channel.send(f"{ctx.user.mention} CONGRATULASHUNS ON SOLVING THE TACTIC!!!")
                
                row = self.session.query(Solved).filter(Solved.server_id==ctx.guild.id, Solved.user_id==ctx.user.id).order_by(Solved.index.desc()).first()
                print("Row:",row)
                ...
                if level=="easy":
                    if row==None:
                        inst1 = Solved(user_id=ctx.user.id,server_id=guild_id,easy=1,medium=0,hard=0,score=10)
                        self.session.add(inst1)
                    else:
                        self.session.query(Solved).filter(Solved.server_id==ctx.guild.id, Solved.user_id==ctx.user.id).update({Solved.easy: Solved.easy+1,Solved.score: Solved.score+10})

                elif level=="medium":
                    if row==None:
                        inst1 = Solved(user_id=ctx.user.id,server_id=guild_id,easy=0,medium=1,hard=0,score=30)
                        self.session.add(inst1)
                    else:
                        self.session.query(Solved).filter(Solved.server_id==ctx.guild.id, Solved.user_id==ctx.user.id).update({Solved.medium: Solved.medium+1,Solved.score: Solved.score+30})

                elif level=="hard":
                    if row==None:
                        inst1 = Solved(user_id=ctx.user.id,server_id=guild_id,easy=0,medium=0,hard=1,score=80)
                        self.session.add(inst1)
                    else:
                        self.session.query(Solved).filter(Solved.server_id==ctx.guild.id, Solved.user_id==ctx.user.id).update({Solved.hard: Solved.hard+1,Solved.score: Solved.score+80})
                
                print("Level:",level)
                test2 = Solved(user_id=ctx.user.id,server_id=guild_id,easy=1,score=10)
                print("After:",test2.easy,test2.medium,test2.hard,test2.score)

                self.session.query(Solving).filter(Solving.channel_id==ctx.channel.id, Solving.user_id==ctx.user.id).delete()
                self.session.commit()
                print("Commit done")
                return
    ...
    async def render(self, message_id, channel_id):
        result = self.session.query(Solving.board).filter(Solving.message_id==message_id, Solving.channel_id==channel_id).all()[-1]
    ...
    async def tactics(self, slash_i):
        await slash_i.response.send_message(content="okey",delete_after=0.0001,ephemeral=True)

        async def select_callback(selected_i):
            await selected_i.response.send_message(content="okey",delete_after=0.0001,ephemeral=True)
            level = selected_i.data["values"][0]  # Get the selected value from the select component
    ...
    async def stats(self, ctx, member: discord.Member):
        ...
        tactic = self.session.query(Solved).filter(Solved.user_id==member.id, Solved.server_id==ctx.guild.id).first()
        games = self.session.query(Games).filter(or_(Games.white_id==member.id, Games.black_id==member.id), Games.server_id==ctx.guild.id).all()
