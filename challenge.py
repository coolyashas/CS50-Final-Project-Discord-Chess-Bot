...
class Challenge(commands.Cog):
    ...
    @slash.command(name="ily", description="The most fundamental secret of the universe")
    ...
    async def play(self, ctx, move: str):
        ...
        self.session.query(Playing).filter(Playing.channel_id==ctx.channel.id).update({Playing.movenum: movenum, Playing.board: board})
        ...
        if board.is_stalemate() or board.is_insufficient_material() or board.is_checkmate():
            if board.result()=="1/2-1/2":
                await ctx.channel.send(f"{w.mention} {board.result()} {b.mention} gg game ended in a draw")
            ...
            inst1 = Games(result=board.result(), white_id=w_id, black_id=b_id, moves=board.move_stack,
                          server_id=ctx.guild.id, start_time=start_time)
            self.session.add(inst1)
            self.session.query(Playing).filter(Playing.channel_id==ctx.channel.id).delete()
        self.session.commit()

    async def render(self, message_id, channel_id):
        result = self.session.query(Playing.board, Playing.movenum).filter(Playing.message_id==message_id, Playing.channel_id==channel_id).first()
    ...
    async def challenge(self, ctx, member: discord.Member):
        ...
        check = self.session.query(Playing).filter(or_(Playing.white_id==ctx.user.id, Playing.black_id==ctx.user.id), Playing.channel_id==ctx.channel.id).first()
        if check:
            await ctx.channel.send(f"{ctx.user.mention}, you already have an ongoing game in this channel")
            return
            
        async def first_callback(tick_i):
            await tick_i.response.send_message(content="okey",delete_after=0.0001,ephemeral=True)
            ...
            if tick_i.data["custom_id"] == "b2":
                await ctx.channel.send(f"{ctx.user.mention}, challenge denied by {member}")
                ...
                message = await ctx.channel.send(embed=embed, view = discord.ui.View(), file=file)


                inst1 = Playing(white_id=w.id, black_id=b.id, message_id=message.id, channel_id=message.channel.id,
                                board=chess.Board(), movenum=0)
                self.session.add(inst1)
                self.session.commit()
        ...
        await ctx.channel.send(f"{member.mention}, {str(ctx.user)[:-2]} challenges you to a game of chess!", view=view)
