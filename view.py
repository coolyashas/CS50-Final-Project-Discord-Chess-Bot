...
class View(commands.Cog):
    ...
    async def button_i(self, i, games):
        ...
        b1.callback = b2.callback = b3.callback = b4.callback = lambda i: self.game_i(i, message.id, message.channel.id)
        view = discord.ui.View().add_item(b1).add_item(b2).add_item(b3).add_item(b4)
        ...
        inst1 = Viewing(board=board, moves=game.moves, movenum=0, embed_id=message.id, channel_id=message.channel.id)
        self.session.add(inst1)
        print("Added to viewing")
        self.session.commit()

    async def game_i(self, i, embed_id, channel_id):
        await i.response.send_message(content="okey",delete_after=0.0001,ephemeral=True)
        #Just need to extract moves of the game from the board object stored for that row
        #in the database

        result = self.session.query(Viewing).filter(Viewing.embed_id==embed_id, Viewing.channel_id==channel_id).first()
        ...
        if i.data["custom_id"] == "b1":
            ...
            self.session.query(Viewing).filter(Viewing.embed_id==embed_id, Viewing.channel_id==channel_id).update(
                {Viewing.board: chess.Board(), Viewing.movenum: 0})
            ...
            self.session.query(Viewing).filter(Viewing.embed_id==embed_id, Viewing.channel_id==channel_id).update(
                {Viewing.board: board, Viewing.movenum: movenum})
    ...
    async def render(self, embed_id, channel_id):
        ...
        result = self.session.query(Viewing.board, Viewing.movenum).filter(Viewing.embed_id==embed_id, Viewing.channel_id==channel_id).first()
    ...
    async def view(self, ctx, member: discord.Member):
        ...
        for index,game in enumerate(games):
            ...
            options = [
                SelectOption(label=f"{w.name} | {b.name} | {game.result} | {game.start_time.strftime('%d/%m/%y %H:%M')}", value=f"B{index}")
            ]
